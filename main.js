...
document.getElementById("btn-parse-plan").addEventListener("click", () => {
  ...
  // Nhận diện dòng tổng: chứa keyword container trong bất kỳ cột nào (POD/Customer/Contract...)
  function detectContainer(row) {
    const joined = [row.pod, row.customer, row.contract, row.index, row.items]
      .map(v => (v||"").toString()).join(" ").toUpperCase();
    if (!/\b(40F|20F|40HC|20GP|40GP|AIR|LCL)\b/.test(joined) && !/\dX\d/.test(joined)) return null;
    // Trích loại cont
    let m = joined.match(/(\d+X\d+\w*|40HC|40GP|20GP|\d{2}F[*×X]?\d*|AIR|LCL)/);
    if (m) return m[0].replace(/[*×Xx]/g,"×").replace(/×$/,"");
    return joined.trim();
  }
  ...
  for (const row of rows) {
    ...
    const hasOrder = itemVal !== "";  // dòng đơn hàng PHẢI có Items (mã hàng)

    // Dòng tổng: KHÔNG có Items, nhưng có chứa container keyword
    if (!hasOrder) {
      const cont = detectContainer(row);
      if (cont && current && current.orders.length) {
    ...
    const o = computeRow({
      ...
      customer: custVal, contract: row.contract||"",
